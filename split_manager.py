            ...
            match exercise_name:
                ...
                case "Shoulder Press":
                    ...
                    if shp.animate_flag:
                        ...
                        shp.run_unity_animator() #TODO need to clear the animation Frames folder after running the animator
                case "Bicep Curl":
                    bic = Curl_mocap()
                    bic.run_mocap(run_from_split= (run_time, animate_flag, delay, set_number))
                    if bic.animate_flag:
                        print("Running the animator now!")
                        bic.run_unity_animator() #TODO need to clear the animation Frames folder after running the animator
                case "Lateral Raise":
                    lar = Lateral_raise_mocap()
                    lar.run_mocap(run_from_split= (run_time, animate_flag, delay, set_number))
                    if lar.animate_flag:
                        print("Running the animator now!")
                        lar.run_unity_animator() #TODO need to clear the animation Frames folder after running the animator
                case _:
